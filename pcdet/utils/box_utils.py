...
def boxes3d_lidar_to_kitti_camera_v2(boxes3d_lidar, cur_calib):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
    ...
    xyz_lidar[:, 2] -= h.reshape(-1) / 2
    xyz_lidar = np.hstack((xyz_lidar, np.ones((xyz_lidar.shape[0], 1), dtype=np.float32)))
    xyz_cam = np.dot(xyz_lidar, cur_calib.T)
...
def boxes3d_lidar_to_kitti_camera_v3(boxes3d_lidar, cur_calib, cur_scale):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
    ...
    """
    boxes3d_lidar_copy = copy.deepcopy(boxes3d_lidar)
    boxes3d_lidar_copy = boxes3d_lidar_copy / cur_scale
    xyz_lidar = boxes3d_lidar_copy[:, 0:3]
    l, w, h = boxes3d_lidar_copy[:, 3:4], boxes3d_lidar_copy[:, 4:5], boxes3d_lidar_copy[:, 5:6]
    r = boxes3d_lidar_copy[:, 6:7]
...
def boxes3d_lidar_to_kitti_camera_v4(boxes3d_lidar, cur_calib, cur_scale, flip_x_enable):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
...
def boxes3d_back_augmentation_to_camera(boxes3d_lidar, calib, rotation, scale, flip_x_enable, noise_rot):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
    ...
    """
    boxes3d_lidar_copy = copy.deepcopy(boxes3d_lidar)
    boxes3d_lidar_copy[:,:6] = boxes3d_lidar_copy[:,:6] / scale
    xyz_lidar = boxes3d_lidar_copy[:, 0:3]
    l, w, h = boxes3d_lidar_copy[:, 3:4], boxes3d_lidar_copy[:, 4:5], boxes3d_lidar_copy[:, 5:6]
    r = boxes3d_lidar_copy[:, 6:7]
    xyz_ori = np.dot(xyz_lidar, rotation.T)
    r -= noise_rot
    if flip_x_enable:
        xyz_ori[:,1] = -xyz_ori[:,1]
        r = -r
    xyz_ori[:, 2] -= h.reshape(-1) / 2
    xyz_cam = calib.lidar_to_rect(xyz_ori)
    r = -r - np.pi / 2

    return np.concatenate([xyz_cam, l, h, w, r], axis=-1)

def boxes3d_back_augmentation_to_camera_v2(boxes3d_lidar, calib, rotation, scale, flip_x_enable, noise_rot):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
...
def back_augmentation(pts, cur_rotation, cur_scale, flip_x_enable):
    pts_copy = copy.deepcopy(pts)
    pts_copy = pts_copy / cur_scale
    xyz_lidar = pts_copy[:, 0:3]
    # xyz_lidar = np.hstack((xyz_lidar, np.ones((xyz_lidar.shape[0], 1), dtype=np.float32)))
    xyz_ori = np.dot(xyz_lidar, cur_rotation.T)
    if flip_x_enable:
        xyz_ori[:,1] = -xyz_ori[:,1]
    return xyz_ori
