...
class AnchorHeadRDIoU(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        if model_cfg.get('requires_grad', True) == False:
    ...
    def get_rdiou_guided_reg_loss(self):
        ...
        box_cls_labels = self.re_box_cls_lables
        batch_size = int(box_preds.shape[0])
        ...
        if box_dir_cls_preds is not None:
            dir_targets = self.forward_ret_dict['dir_targets']
    ...
    def forward(self, data_dict):
        ...
        h,w = spatial_features_2d.shape[-2], spatial_features_2d.shape[-1]
        ...
        # Extract the parameters of conv_box & conv_cls
        data_dict['bev_conv_box_weight'] = self.conv_box.weight
        data_dict['bev_conv_box_bias'] = self.conv_box.bias
        data_dict['bev_conv_cls_weight'] = self.conv_cls.weight
        data_dict['bev_conv_cls_bias'] = self.conv_cls.bias
        data_dict['bev_conv_box_dir_cls_weight'] = self.conv_dir_cls.weight
        data_dict['bev_conv_box_dir_cls_bias'] = self.conv_dir_cls.bias

        if isinstance(self.anchors, list): # kitti: true
            if self.use_multihead:
                anchors = torch.cat(
                    [anchor.permute(3, 4, 0, 1, 2, 5).contiguous().view(-1, anchor.shape[-1]) for anchor in
                    self.anchors], dim=0)
        ...
        if self.training:
            ...
            self.re_box_cls_lables, self.pse_box_cls_labels = self.get_clsreg_targets()
            
            dir_targets = self.get_direction_target(
                anchors, self.forward_ret_dict['box_reg_targets'],
                dir_offset=self.model_cfg.DIR_OFFSET,
                num_bins=self.model_cfg.NUM_DIR_BINS
            )
            self.forward_ret_dict['dir_targets'] = dir_targets

            # ipdb.set_trace()

            data_dict['re_box_cls_lables'] = self.re_box_cls_lables.view(batch_size, h, w, -1)
            data_dict['rpn_pse_box_cls_labels'] = self.pse_box_cls_labels.view(batch_size, h, w, -1)
            data_dict['rpn_box_reg_targets'] = self.forward_ret_dict['box_reg_targets'].view(batch_size, h, w, -1)
            data_dict['rpn_dir_targets'] = dir_targets.view(batch_size, h, w, -1)
        data_dict['rpn_anchors'] = anchors.view(batch_size, h, w, -1)
