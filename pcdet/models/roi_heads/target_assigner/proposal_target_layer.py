...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        Returns:
            batch_dict:
                ...
                rcnn_cls_labels: (B, M)
        """
        batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, batch_gt_box_aug_mask = self.sample_rois_for_rcnn(
        ...
        targets_dict = {'rois': batch_rois, 'gt_of_rois': batch_gt_of_rois, 'gt_iou_of_rois': batch_roi_ious,
                        ...
                        'gt_box_aug_mask': batch_gt_box_aug_mask}
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        gt_box_aug_mask = batch_dict['gt_box_aug_mask']

        code_size = rois.shape[-1]
        batch_rois = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE, code_size)
        batch_gt_of_rois = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE, code_size + 1)
        batch_gt_box_aug_mask = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE, 1)
        batch_roi_ious = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE)
        batch_roi_scores = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE)
        batch_roi_labels = rois.new_zeros((batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE), dtype=torch.long)

        for index in range(batch_size):
            cur_roi, cur_gt, cur_roi_labels, cur_roi_scores, cur_gt_box_aug_mask = \
                rois[index], gt_boxes[index], roi_labels[index], roi_scores[index], gt_box_aug_mask[index]
            k = cur_gt.__len__() - 1
            ...
            cur_gt_box_aug_mask = cur_gt_box_aug_mask[:k + 1]
            cur_gt_box_aug_mask = cur_gt_box_aug_mask.new_zeros((1, cur_gt_box_aug_mask.shape[1])) if len(cur_gt_box_aug_mask) == 0 else cur_gt_box_aug_mask
            ...
            batch_gt_box_aug_mask[index] = cur_gt_box_aug_mask[gt_assignment[sampled_inds]]

        return batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, batch_gt_box_aug_mask
