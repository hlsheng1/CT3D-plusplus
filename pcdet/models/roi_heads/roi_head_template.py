...
class RoIHeadTemplate(nn.Module):
    ...
    def assign_targets_part(self, batch_dict):

        with torch.no_grad():
            targets_dict = self.proposal_target_layer.forward(batch_dict)

        gt_box_aug_mask = targets_dict['gt_box_aug_mask']
        targets_dict['gt_box_aug_mask'] = gt_box_aug_mask.detach()
        return targets_dict

    def gt_box_transfer(self, batch_size, targets_dict):
        rois = targets_dict['rois']  # (B, N, 7 + C)
        gt_of_rois = targets_dict['gt_of_rois']  # (B, N, 7 + C + 1)
        targets_dict['gt_of_rois_src'] = gt_of_rois.clone().detach()
