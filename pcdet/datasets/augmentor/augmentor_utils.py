...
# This is wrong code.
def random_flip_along_x_both_lidar_and_image(gt_boxes, points, images, return_flip=False): # wrong code
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
    ...
    if enable:
        ...
        # center_points_of_lidar = np.zeros(3)
        # img_pts, img_depth = calib.lidar_to_img(center_points_of_lidar)
        # W = image.shape[1]
        ...
        if gt_boxes.shape[1] > 7:
            gt_boxes[:, 8] = -gt_boxes[:, 8]
    if return_flip:
        return gt_boxes, points, images, enable
    return gt_boxes, points, images
...
def global_rotation_add_calib(gt_boxes, points, rot_range, return_rot=False):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    points, rot_matrix = common_utils.rotate_points_along_z_return_calib(points[np.newaxis, :, :], np.array([noise_rotation]))
    ...
    # rot_matrix = np.hstack((rot_matrix, np.zeros((3, 1), dtype=np.float32)))  # (3, 4)
    # rot_matrix = np.vstack((rot_matrix, np.array([0, 0, 0, 1], dtype=np.float32)))  # (4, 4)
    # trans_lidar_to_cam_refine = trans_lidar_to_cam @ rot_matrix

    if return_rot:
        return gt_boxes, points, noise_rotation, rot_matrix
    return gt_boxes, points, rot_matrix
...
def global_rotation_rcnn(gt_boxes, points, rois, rot_range, return_rot=False):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    gt_boxes[:, 0:3] = common_utils.rotate_points_along_z(gt_boxes[np.newaxis, :, 0:3], np.array([noise_rotation]))[0]
    gt_boxes[:, 6] += noise_rotation
    rois[:, 0:3] = common_utils.rotate_points_along_z(rois[np.newaxis, :, 0:3], np.array([noise_rotation]))[0]
    rois[:, 6] += noise_rotation
    ...
    if return_rot:
        return gt_boxes, points, rois, noise_rotation
    return gt_boxes, points, rois


def global_scaling_rcnn(gt_boxes, points, rois, scale_range, return_scale=False):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading]
        points: (M, 3 + C),
    ...
    rois[:, :6] *= noise_scale
    if return_scale:
        return gt_boxes, points, rois, noise_scale
    return gt_boxes, points, rois
