...
class DataAugmentor(object):
    ...
    def random_world_flip_lidar_img(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_world_flip_lidar_img, config=config)
        gt_boxes, points = data_dict['gt_boxes'], data_dict['points']
        images = data_dict["images"]
        # calib = data_dict["calib"]
        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['x', 'y']
            gt_boxes, points, images, enable = getattr(augmentor_utils, ('random_flip_along_%s'% cur_axis +'_both_lidar_and_image'))(
                gt_boxes, points, images, return_flip=True
            )
            data_dict['flip_%s'%cur_axis] = enable
    ...
    def random_world_rotation_add_calib(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_world_rotation_add_calib, config=config)
        rot_range = config['WORLD_ROT_ANGLE']
        if not isinstance(rot_range, list):
            rot_range = [-rot_range, rot_range]

        gt_boxes, points, noise_rot, aug_rotation = augmentor_utils.global_rotation_add_calib(
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        #  print(data_dict['trans_lidar_to_cam'])
    ...
    def do_nothing(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.do_nothing, config=config)
        return data_dict
...
# def random_world_rotation_rcnn(points, gt_boxes, roi, config):
#     rot_range = config['WORLD_ROT_ANGLE']
#     if not isinstance(rot_range, list):
#         rot_range = [-rot_range, rot_range]
#     gt_boxes, points, roi, noise_rot = augmentor_utils.global_rotation_rcnn(
#         gt_boxes, points, roi, rot_range=rot_range, return_rot=True
#     )
#     return gt_boxes, points, roi, noise_rot

# def random_world_scaling_rcnn(points, gt_boxes, roi, config):
#     gt_boxes, points, roi, noise_scale = augmentor_utils.global_scaling_rcnn(
#          gt_boxes, points, roi, config['WORLD_SCALE_RANGE'], return_scale=True
#     )
#     return gt_boxes, points, roi, noise_scale