...
class OptimWrapper():
    ...
    @classmethod
    def create_new(cls, opt_func, lr,
               layer_groups0, layer_groups1, **kwargs):
        "Create an `optim.Optimizer` from `opt_func` with `lr`. Set lr on `layer_groups`."
        split_groups0 = split_bn_bias(layer_groups0)
        split_groups1 = split_bn_bias(layer_groups1)
        # ipdb.set_trace()
        input0 = [{'params': trainable_params(l), 'lr': lr} for l in split_groups0]
        input1 = [{'params': trainable_params(l), 'lr': lr*3} for l in split_groups1]
        input = input0+input1
        opt = opt_func(input)
        opt = cls(opt, **kwargs)
        # ipdb.set_trace()
        opt.lr, opt.opt_func = listify(lr, layer_groups0)+listify(lr*3, layer_groups1), opt_func
        return opt
