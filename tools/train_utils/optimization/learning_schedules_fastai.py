...
class OneCycle_split(LRSchedulerStep):
    def __init__(self, fai_optimizer, total_step, lr_max, moms, div_factor,
                 pct_start, num_roi_head, num_all):
        self.num_roi_head = num_roi_head
    ...
    def step(self, step):
        for start, end, func in self.lr_phases:
            if step >= start:
                # ipdb.set_trace()
                input = [func((step - start) / (end - start))] * self.num_all
                input[-self.num_roi_head] = input[-self.num_roi_head]*3
                self.optimizer.lr = input
        for start, end, func in self.mom_phases:
            if step >= start:
                self.optimizer.mom = func((step - start) / (end - start))
